...
## Lesson 2 of 4 — CJA MCP Server

est completion: 20 min *(Hands-on in Cursor — two parts)*
...
- Explain what MCP is and how it enables AI-to-tool connectivity
- Connect Cursor to the CJA MCP Server via OAuth
...
Think of it as a standard plug. The CJA MCP Server is a socket. Any AI client that supports the standard can plug in.
...
### 2.3 Exercise: Connect to the CJA MCP Server

The CJA MCP Server is hosted at `https://mcp-gateway.adobe.io/cja/mcp`. You connect once per Cursor installation and authenticate with your Adobe IMS credentials.

1. Open **Cursor**
2. Navigate to **Settings → Tools & MCP → Add New MCP Server**
3. An editor opens with `mcp.json`. Add the following entry inside `"mcpServers"` and save the file:
   ...
   ```
4. Back in **Settings → Tools & MCP**, find the **CJA** server entry and click **Connect**
5. A dialog asks for permission to open the authentication page — click **Open**
6. Your browser opens the Adobe Experience Platform login page. Sign in with your lab credentials.
7. Select your account type and choose your organization when prompted.
8. After authentication, the browser shows a dialog asking to open Cursor — click **Open Cursor.app**
9. Return to Cursor. The CJA server entry should now show as connected, with tools listed below it.
...
3. The agent should respond with a list of available tools — there are 25 in total.

### 2.4 Exercise: Create a Workspace with Natural Language
...
2. Load the project builder skill by typing:

   ```
   @cja-project-builder
   ```
3. **Prompt 1 — Discover your environment:**

   ```
   What data views do I have access to?
   ```

   The agent calls `findDataViews` and returns a list. Note the name of the data view you want to use.
4. **Prompt 2 — Set your default data view:**

   ```
   Set [Data View Name] as my default data view for this session.
   ```

   Replace `[Data View Name]` with the name from the previous response. The agent calls `setDefaultSessionDataViewId` — now every subsequent tool call uses this data view by default.
5. **Prompt 3 — Describe the workspace:**

   ```
   Create an e-commerce performance dashboard for the Luma retail data.
   Show revenue, orders, and conversion rate for the last 30 days,
   broken down by product category. Include a line chart for the daily revenue trend.
   ```
6. The agent responds with a proposed structure — panels, visualizations, dimensions, metrics. Review it. If something looks wrong, ask for a change before confirming.
7. **Confirm:**

   ```
   Yes, go ahead and create it.
   ```
8. The agent calls `upsertProject`. It will return a project ID and name.
9. Open **CJA → Workspace** and find your new project. Verify the panels match what you asked for.

> **Tip:** Be specific in your prompts. Include time ranges ("last 30 days"), metric names ("orders", "revenue"), and dimensions ("product category") when you know them. You can also ask follow-ups: *"Can you add a segment for mobile users to the first panel?"*
